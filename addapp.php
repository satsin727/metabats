...
//			$rate = $_POST['rate'];
		...
		else
		{
			...
			{
				$qcid = "select * from consultants where status = 1";
			}
			else 
			{ 
				$rmid = $conn->query("select rmid from users where uid = $sessid")->fetchColumn();
				
				$qcid = "select * from assigned AS A INNER JOIN consultants AS B ON A.cid = B.cid where uid = $rmid and B.skill = $skillid and B.status = 1"; 
			}	
