...
$data = $ins->fetchAll();
                    ...
                    foreach($data as $row) {
                        ...
                        $comments = "";
                            ...
                            $totalrc = $conn->query("select count(*) from app_data as A Left Join req as B ON A.reqid  = B.reqid where B.ureq_id = '$ureq_id' and A.rcdone = 1 and B.datetime > '$curdate' ") ;
                            if($totalrc>0)
                            {
                                $reqstatus = "Utilized";
                            }
